...
- **Generate** — SA (4m), CL (10m), MC (1m), Practical (10m) questions from any supported legislation section
...
Five acts are currently synced from legislation.govt.nz:

| Act | Sections |
|-----|----------|
| Crimes Act 1961 | 575 |
| Search and Surveillance Act 2012 | 447 |
| Misuse of Drugs Act 1975 | 120 |
| Evidence Act 2006 | 256 |
| Arms Act 1983 | 264 |

To add a new act, append an entry to the `ACTS` array in `scripts/sync-legislation.ts` and run `npm run sync-legislation`.
...
- [x] MC HTML rendering — stems and options containing HTML are rendered correctly in the library view and stripped cleanly in Markdown and Totara XML exports
